...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="读取 unsteady.xlsx，基于 solve/ 中已有稳态 case+data 继续做非稳态计算。支持 Slurm array 单工况模式。"
    )
    parser.add_argument(
        "--base-dir",
        type=Path,
        default=Path(__file__).resolve().parent,
        help="项目根目录。默认是当前 main_unsteady.py 所在目录。",
    )
    parser.add_argument(
        "--excel",
        type=str,
        default="unsteady.xlsx",
        help="非稳态参数表文件名。默认 unsteady.xlsx。",
    )
    parser.add_argument(
        "--sheet",
        type=str,
...
def main() -> None:
    ...
    print("=" * 72)
    print("Fluent 2022 R1 非稳态续算")
    print(f"项目目录         : {config.base_dir}")
    print(f"参数表           : {solver.excel_path}")
    ...
    print(f"默认核数         : {config.processor_count}")
